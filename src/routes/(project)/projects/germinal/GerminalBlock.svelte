...
<script lang="ts">
    ...
    const DEMO_ROLES = [
        {
            labelKey: 'germinal.demo_public_label',
            descKey: 'germinal.demo_public_desc',
            url: 'https://germinal.henga.dev',
            email: null,
            password: null,
        },
        {
            labelKey: 'germinal.demo_admin_label',
            descKey: 'germinal.demo_admin_desc',
            url: 'https://germinal.henga.dev/login',
            email: 'admin@demo.henga.dev',
            password: 'password',
        },
        {
            labelKey: 'germinal.demo_staff_label',
            descKey: 'germinal.demo_staff_desc',
            url: 'https://germinal.henga.dev/login',
            email: 'staff@demo.henga.dev',
            password: 'password',
        },
    ];
</script>
...
<ProjectDetail
    ...
    demoAnchor="explore-demo"
    githubUrl="https://github.com/hengadev/germinal"
>
    ...
    <section class="story-section demo-section" id="explore-demo">
        <h2 class="section-title">{$_('detail.explore_demo_title')}</h2>
        <p class="demo-subtitle">{$_('detail.explore_demo_subtitle')}</p>
        <div class="role-grid">
            {#each DEMO_ROLES as r}
            <div class="role-card">
                <div class="role-card__header">
                    <span class="role-label">{$_(r.labelKey)}</span>
                    <a href={r.url} target="_blank" rel="noopener noreferrer" class="open-btn">
                        {$_('detail.open_role')} <ArrowUpRight size={12} />
                    </a>
                </div>
                <p class="role-desc">{$_(r.descKey)}</p>
                {#if r.email}
                <div class="credentials">
                    <span class="cred-item"><span class="cred-key">email</span>{r.email}</span>
                    <span class="cred-item"><span class="cred-key">password</span>{r.password}</span>
                </div>
                {:else}
                <div class="credentials">
                    <span class="no-login">{$_('detail.no_login_required')}</span>
                </div>
                {/if}
            </div>
            {/each}
